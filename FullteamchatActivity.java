...
public class FullteamchatActivity<User> extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView((int) C0250R.layout.activity_fullteamchat);
        getWindow().setSoftInputMode(3);
        this.groupChatName = getIntent().getExtras().get("groupName").toString();
        ...
        this.sendButton.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                FullteamchatActivity.this.saveMessageInToDatabase();
                FullteamchatActivity.this.messageArea.setText("");
                FullteamchatActivity.this.scrollView.fullScroll(130);
            }
        });
        this.groupNameReference.addChildEventListener(new ChildEventListener() {
            public void onChildAdded(DataSnapshot snapshot, String previousChildName) {
                if (snapshot.exists()) {
                    FullteamchatActivity.this.DisplayMessages(snapshot);
                }
            }
    ...
    public void saveMessageInToDatabase() {
        this.message = this.messageArea.getText().toString();
        ...
        messageInfoMap.put("message", this.message);
    ...
    private void getUserInfo() {
        this.userReference.child(this.currentUserId).addValueEventListener(new ValueEventListener() {
            public void onDataChange(DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    String unused = FullteamchatActivity.this.currentUserName = snapshot.child("name").getValue().toString();
                }
            }
